 ...
 */
public class LexicalTransferEngineHybrid {
     ...
     */
    private static Logger logger = Logger.getLogger(LexicalTransferEngineHybrid.class);
    /**
    ...
    private static String PUNC = "pun";
	...
	public static String smt_output = null;
    /**
    ...
    public void initialize(String USER_HOME, boolean openUserDict, boolean openSessionDict, boolean openSystemDict,
            boolean openNNPDict, boolean openBilingualDict, String smt_output) throws MultilingualDictException {
    ...
    public void translate(String inputFileName,
            String outputFileName, Language sourceLanguage, Language targetLanguage) throws FileNotFoundException, IOException, MultilingualDictException {
     ...
     */
    private void transfer(String inputFileName, String outputFileName, Language sourceLanguage, Language targetLanguage) throws MultilingualDictException,FileNotFoundException, IOException{
        try {
            String chunk_name = "";
			...
			HashMap<String,String> phrasemappings = new HashMap<String,String>();
            story.readFile(inputFileName);
			BufferedReader smt_output_reader = new BufferedReader(new FileReader(new File(this.smt_output)));
			String curr_smt_out = null;
			String curr_chunk_split = null;
            int scount = story.countSentences();//scount store the number of sentences ( no of {<Sentence id="1">,</Sentence>}) in input file 
            for (int i = 0; i < scount; i++) {
				curr_smt_out=smt_output_reader.readLine().trim();
				//System.out.println(curr_smt_out);
				//System.out.println(curr_smt_out.split("\\|").length);
				curr_chunk_split=smt_output_reader.readLine().trim();
				//System.out.println(curr_chunk_split);
				//System.out.println(curr_chunk_split.split(" ").length);
				for(int smt_i=0;smt_i<curr_smt_out.split("\\|").length;smt_i=smt_i+2){
					String out_phrase=curr_smt_out.split("\\|")[smt_i];
					out_phrase=out_phrase.replace("।","").trim();
					String range=curr_smt_out.split("\\|")[smt_i+1];
					int start=Integer.parseInt(range.split("-")[0]);
					int end=Integer.parseInt(range.split("-")[1]);
					String in_phrase="";
					for(int phr_i=start;phr_i<=end;phr_i++){
						in_phrase=in_phrase+curr_chunk_split.split(" ")[phr_i]+" ";
					}
					in_phrase=in_phrase.replace(".","");
					phrasemappings.put(in_phrase.trim(),out_phrase.trim());
				}
				//System.out.println(phrasemappings);
...
//			System.out.println(sourceWord+" "+isKridanta);
						//System.out.println(sourceWord.replaceAll("णे$","")+" "+sourceTam+"-"+phrasemappings.get((sourceWord.replaceAll("णे$","")+" "+sourceTam).trim())); 		
                        ...
                        boolean tildaPrefixWord = sourceWord.startsWith(GlobalConstants.TILDA);
                         tamkey=(sourceWord.replaceAll("णे$","")+" "+sourceTam.replaceAll("_"," ").replaceAll("\\+"," ").replaceAll("#"," ")).trim();
                        if((chunkNameType.contains("VG") || lcat.equalsIgnoreCase("pn"))&& tamkey.split(" ").length>1){ //prefer smt translations of bigger phrases
							if(phrasemappings.containsKey(tamkey)){
								
								vg_translation = phrasemappings.get(tamkey);
								if(vg_translation.split(" ").length<2){ //no suffix if only single word translation
									vg_lex = vg_translation;
								}
								else{ //if more than 1 token as a translation then treat the rest as as suffix
									vg_lex = vg_translation.split(" ")[0];
									vg_tam = vg_translation.substring(vg_translation.indexOf(" ")+1,vg_translation.length());
									//System.out.println(vg_lex+" "+vg_tam);
								}
								...
								"/bin/sh",
								"-c",
								"echo \""+tamkey+"\" | /bin/nc localhost 12345"
								};
								...
								while ((line = b.readLine()) != null) {
									...
									for(int smt_ctr=0;smt_ctr<line.split("\\|").length;smt_ctr=smt_ctr+2){
										vg_translation = vg_translation+line.split("\\|")[smt_ctr].trim()+" ";
									}
									vg_translation = vg_translation.trim();
								}
								if(vg_translation.split(" ").length<2){
									vg_lex = vg_translation;
								}
								else{
									vg_lex = vg_translation.split(" ")[0];
						...
						vg_tam = vg_tam.replaceAll(" ","_").replaceAll(",",";");
						} 
				...
				targetTams[0] = "का_आगे";
                        ...
                        if (targetWords == null || targetWords.length == 0) {
                            ...
                            if (nodefaWord != null) {
                                ...
                                if(vg_lex.length()>0){
									if(lcat.equalsIgnoreCase("pn")){
										newaTagetWord.setValue(vg_lex);
									} else
										newaTagetWord.setValue(AT+vg_lex);
								} else{
                    ...
                    for (int k = 0; k < node.countChildren(); k++) {
                        ...
                        if (nodeChild.getFeatureStructures() != null && nodeChild.getFeatureStructures().getChildCount() > 0) {
							...
							//System.out.println(nodefapos.getAltValue(0).toString());
                            if (usePOSTags) {
                            ...
                            if (targetWords == null || targetWords.length == 0) {
                                if (caratPrefixWord || tildaPrefixWord) {
                                    ...
                                    if (nodefaWord != null) {
                                        ...
                                        if(vg_lex.length()>0 ){
											if(lcat.equalsIgnoreCase("pn")){
											newalue.setValue(vg_lex);
										} else
											newalue.setValue(AT+vg_lex);
										} else {
									...
									if (nodefaWord != null && nodefapos.getAltValue(0).toString().equals(PUNC)){
                                    ...
                                    if (!nodefapos.getAltValue(0).toString().equals(PUNC) && !sourceWord.trim().equals("")) {
                                        if(vg_lex.length()>0){
											if(lcat.equalsIgnoreCase("pn")){
												nodeChild.setLexData(vg_lex);
											} else 
												nodeChild.setLexData(AT+vg_lex);
										} else{
											nodeChild.setLexData(AT + sourceWord);
										}
                                        try {
								...
								//System.out.println(nodefapos.getAltValue(0).toString());
                                logger.debug("Translation for " + sourceWord + " is " + targetWords[0]);
                                if(vg_lex.length()>0 && !nodefapos.getAltValue(0).toString().equals(PUNC)){
									if(lcat.equalsIgnoreCase("pn")){
												nodeChild.setLexData(vg_lex);
											} else 
												nodeChild.setLexData(AT+vg_lex);
								} else{
									if(nodefapos.getAltValue(0).toString().equals(PUNC)){
										nodeChild.setLexData(AT+targetWords[0]);
									} else{
                                ...
                                if (nodefaWord != null) {
									...
									if(lcat.equalsIgnoreCase("pn")){
										...
										if(nodefapos.getAltValue(0).toString().equals(PUNC)){
											newalue.setValue(AT+targetWords[0]);
									} else{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     * @throws IOException
     */
    public static void main(String args[]) throws MultilingualDictException, IOException, FileNotFoundException {
        ...
        Getopt g = new Getopt("LexicalTransferEngineHybrid", args, "-:i:o:m:s:t:d:u:y::e::r::n::l::p::j:h", longopts);
        g.setOpterr(false);
        while ((c = g.getopt()) != -1) {
            switch (c) {
                case 0:
        ...
        LexicalTransferEngineHybrid lte = new LexicalTransferEngineHybrid();
        lte.initialize(USER_HOME, openUserDict, openSessionDict, openSystemDict, openNNPDict, openBilingualDict, smt_output);
