...
public class IndexReader {
	...
	public IndexReader(String root) throws MultilingualDictException {
		for(int i=0; i<Language.ALL_LANGS.length; i++) {
			for(int j=0; j<POS1.CATEGORIES.length; j++) {
	...
	public IndexReader(String root, Language sourceLanguage, Language targetLanguage) throws MultilingualDictException {
		for(int j=0; j<POS1.CATEGORIES.length; j++) {
			wordReaders.put(sourceLanguage + "_" + POS1.CATEGORIES[j], getWordReader(root, sourceLanguage, j));
			wordReaders.put(targetLanguage + "_" + POS1.CATEGORIES[j], getWordReader(root, targetLanguage, j));
			
			crossLinksReaders.put(sourceLanguage + "_" + POS1.CATEGORIES[j], getCrossLinksReader(root, sourceLanguage, j));
			crossLinksReaders.put(targetLanguage + "_" + POS1.CATEGORIES[j], getCrossLinksReader(root, targetLanguage, j));

			synsetReaders.put(sourceLanguage + "_" + POS1.CATEGORIES[j], getSynsetReader(root, sourceLanguage, j));
			synsetReaders.put(targetLanguage + "_" + POS1.CATEGORIES[j], getSynsetReader(root, targetLanguage, j));
		}
	...
	public String[] getCrossLinkedWords(String lemma, Language sourceLanguage,
		...
		for(int i=0; i<POS1.CATEGORIES.length; i++) {
			String[] crossLinkedWords = getCrossLinkedWords(
					lemma, sourceLanguage, targetLanguage, POS1.CATEGORIES[i]);
	 ...
	 */
	public String[] getCrossLinkedWords(String lemma, Language sourceLanguage,
			Language targetLanguage, POS1 pos)
			throws MultilingualDictException {
	...
	throws MultilingualDictException {
		...
		for(int i=0; i<POS1.CATEGORIES.length; i++) {
			String[] crosssLinkedWords = getCrossLinkedWords(
					lemma, sourceLanguage, targetLanguage, synsetId, POS1.CATEGORIES[i]);
	 ...
	 */
	public String[] getCrossLinkedWords(
			String lemma, Language sourceLanguage, Language targetLanguage, String synsetId, POS1 pos)
	throws MultilingualDictException {
	...
	throws MultilingualDictException {
		...
		for(int i=0; i<POS1.CATEGORIES.length && record == null; i++) {
			record = getDSFRecord(synsetId, POS1.CATEGORIES[i], language);
		}
		return record;
	}
	...
	throws MultilingualDictException {
		...
		for(int i=0; i<POS1.CATEGORIES.length; i++) {
			DSFRecord[] records = getDSFRecords(lemma, POS1.CATEGORIES[i], language);
			if(records == null)
				continue;
...
//System.out.println(pos);
	...
	public void close(Language sourceLanguage, Language targetLanguage) throws MultilingualDictException {
		for(int j=0; j<POS1.CATEGORIES.length; j++) {
			wordReaders.get(sourceLanguage + "_" + POS1.CATEGORIES[j]).close();
			wordReaders.get(targetLanguage + "_" + POS1.CATEGORIES[j]).close();
			
			crossLinksReaders.get(sourceLanguage + "_" + POS1.CATEGORIES[j]).close();
			crossLinksReaders.get(targetLanguage + "_" + POS1.CATEGORIES[j]).close();
	
			synsetReaders.get(sourceLanguage + "_" + POS1.CATEGORIES[j]).close();
			synsetReaders.get(targetLanguage + "_" + POS1.CATEGORIES[j]).close();
		}
	}
	/**
	 * <p><b>Method</b> 	: getKey
	...
	private String getKey(int i, int j) {
		return Language.ALL_LANGS[i] + "_" + POS1.CATEGORIES[j];
